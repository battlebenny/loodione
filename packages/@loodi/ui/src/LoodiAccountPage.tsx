...
type SheetName = 'email' | 'color' | 'google' | 'signout' | 'delete' | null
...
function Row({ icon, label, detail, onClick, last = false, className, detailClassName, iconClassName, iconStyle }: { icon: ReactNode; label: string; detail: string; onClick?: () => void; last?: boolean; className?: string; detailClassName?: string; iconClassName?: string; iconStyle?: CSSProperties }) {
  return <button type="button" className={`loodi-account__row${last ? ' loodi-account__row--last' : ''}${className ? ` ${className}` : ''}`} onClick={onClick} disabled={!onClick}>
    <span className={`loodi-account__icon${iconClassName ? ` ${iconClassName}` : ''}`} style={iconStyle}>{icon}</span>
    <span className="loodi-account__copy"><strong>{label}</strong><small className={detailClassName}>{detail}</small></span>
...
export function LoodiAccountPage({ isAuthenticated, playerName, profileColor, email, hasGoogleIdentity = false, onCreateAccount, onSignIn, onSignOut, onDeleteAccount, onUpdateEmail, onUpdateProfileColor, onLinkGoogleIdentity, onUnlinkGoogleIdentity, onOverlayChange, successMessage }: LoodiAccountPageProps) {
  ...
  return <main className="loodi-account">
    ...
    {sheet === 'delete' && <Sheet title="Supprimer le compte" onClose={() => setSheet(null)}><p>Cette action est définitive. Ton identité, ton profil et toutes tes données Loodi seront supprimés.</p><label htmlFor="loodi-account-delete-confirmation">Écris SUPPRIMER pour confirmer</label><input id="loodi-account-delete-confirmation" disabled={deletingAccount} value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} autoComplete="off" /><button type="button" className="loodi-account__delete" disabled={deletingAccount || deleteConfirmation !== 'SUPPRIMER'} onClick={() => { if (!onDeleteAccount || deletingAccount) return; setDeletingAccount(true); void onDeleteAccount().then(() => setSheet(null)).catch(showError).finally(() => setDeletingAccount(false)) }}>{deletingAccount ? 'Suppression…' : 'Supprimer définitivement'}</button></Sheet>}
  </main>
}
