...
export function Settings({
  ...
  const favAppName = favoriteAppId ? (apps.find((a) => a.id === favoriteAppId)?.name ?? favoriteAppId) : 'Aucune';
  ...
  if (accountOpen)
    return (
      <div
        data-testid="loodi-account-scroll-container"
        onScroll={(event) => onAccountScrollProgress?.(Math.min(event.currentTarget.scrollTop / 52, 1))}
        className="fixed inset-0 z-20 overflow-y-auto bg-[var(--color-settings-bg)] px-4 pt-14 text-[var(--color-text-light)] dark:bg-[var(--color-settings-bg-dark)] dark:text-[var(--color-text-primary-dark)]"
        style={{
      ...
      >
        <LoodiAccountPage
          ...
          profileColor={accountProfileColor}
          email={auth.email}
          ...
          onUpdateProfileColor={auth.updateProfileColor}
          onLinkGoogleIdentity={onLinkGoogleIdentity ?? auth.linkGoogleIdentity}
          onUnlinkGoogleIdentity={onUnlinkGoogleIdentity ?? auth.unlinkGoogleIdentity}
  ...
  return (
    <div className="fixed inset-0 z-20 bg-[var(--color-settings-bg)] dark:bg-[var(--color-settings-bg-dark)] text-[var(--color-text-light)] dark:text-[var(--color-text-primary-dark)] flex flex-col">
      ...
      >
        <h1
          style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.25rem, 4vw, 1.75rem)', textWrap: 'balance' }}
        >
          Paramètres généraux
        </h1>
      </div>
          ...
          <div className="mt-5">
            <SectionCard>
              ...
              <Row
                ...
                secondary={
                  configuredLocalModuleCount
                    ? `${configuredLocalModuleCount} URL${configuredLocalModuleCount > 1 ? 's' : ''} configurée${configuredLocalModuleCount > 1 ? 's' : ''}`
                    : 'Configurer les URL de test'
                }
      ...
      <BottomSheet open={moduleSheetOpen} onClose={() => setModuleSheetOpen(false)} title="Modules locaux">
        <div className="space-y-4 pb-2">
          <p className="text-sm leading-5 text-black/55 dark:text-white/55">
            Saisis l’URL complète de chaque PWA. Utilise l’IP de ton Mac sur appareil physique, ou 10.0.2.2 dans
            l’émulateur Android.
          </p>
          {developmentApps?.map((app) => (
            <label key={app.id} className="block">
              <span className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-black/45 dark:text-white/45">
                {app.name}
              </span>
              <input
                aria-label={app.name}
