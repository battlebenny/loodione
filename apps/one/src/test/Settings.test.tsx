...
describe('Settings', () => {
  ...
  it('offers the Loodi account from global settings', () => {
    render(
      <Settings onClose={vi.fn()} apps={[]} favoriteAppId={null} onFavoriteChange={vi.fn()} initialPage="account" />,
    );
  ...
  it('shows a confirmation toaster after a Google identity is associated', () => {
    render(
      <Settings
        ...
        initialPage="account"
        isAuthenticated
        accountSuccessMessage="Ton compte Google est maintenant associé."
      />,
    );
  ...
  it('lets a signed-in player choose a profile colour from their identity settings', () => {
    ...
    render(
      <LoodiAccountPage
        isAuthenticated
        playerName="battle_benny"
        profileColor="#A83D16"
        onCreateAccount={vi.fn()}
        onSignIn={vi.fn()}
        onUpdateProfileColor={onUpdateProfileColor}
      />,
    );
  ...
  it('closes the Google sheet before showing an association error', async () => {
    ...
    render(
      <LoodiAccountPage
        ...
        onCreateAccount={vi.fn()}
        onSignIn={vi.fn()}
        onLinkGoogleIdentity={vi.fn().mockRejectedValue(new Error('Échec Google'))}
        onOverlayChange={onOverlayChange}
      />,
    );
  ...
  it('closes an account bottom sheet when its handle is dragged down', () => {
    render(
      <LoodiAccountPage isAuthenticated onCreateAccount={vi.fn()} onSignIn={vi.fn()} onLinkGoogleIdentity={vi.fn()} />,
    );
  ...
  it('lets a player unlink Google without removing their e-mail identity', () => {
    ...
    render(
      <LoodiAccountPage
        ...
        onCreateAccount={vi.fn()}
        onSignIn={vi.fn()}
        onUnlinkGoogleIdentity={onUnlinkGoogleIdentity}
      />,
    );
  ...
  it('requires an explicit confirmation before deleting the account', () => {
    ...
    expect(screen.getByRole('button', { name: /la fosse des joueurs damnés/i })).toHaveAttribute(
      'aria-expanded',
      'true',
    );
  ...
  it('renders absolute module icons as images in the favourite picker', () => {
    render(
      <Settings
        onClose={vi.fn()}
        apps={[
    ...
    fireEvent.click(screen.getByRole('button', { name: /application préférée/i }));
    expect(screen.getByAltText(/^collec$/i)).toHaveAttribute(
      'src',
