...
### Accent secondaire
- **Jaune pion** (`#F5C842`) : accent de marque secondaire, réservé aux badges, highlights et détails décoratifs. Il n’est jamais associé à du texte blanc : le texte et les icônes utilisent le navy `#1b2a4a` (contraste 8,95:1). Il ne remplace pas l’orange brûlé pour les actions principales.

### Neutral
- **Beige** (`#f1f1ef` / oklch(0.94 0.006 85)) : fond du shell en mode clair.
...
Les fonds de page réutilisent `--color-bg-light` (`#F1F1EF`) et `--color-bg-dark` (`#1A1A18`) : il n’existe pas de token `background-page`. Il n’existe volontairement pas non plus de token `surface-base` ; les surfaces opaques utilisent `bg-white` en clair et `bg-white/5` en sombre.
...
- **Capacité:** cinq éléments au maximum : jusqu’à quatre onglets fournis par le module, puis le bouton fixe **Loodi**.
- **Bouton Loodi:** dernier élément, icône `grid-2x2` Lucide + label « Loodi » ; ouvre le launcher « Toutes les applications ».
- **Tabs:** colonne de largeur égale, hauteur 40px ; icône Lucide 20px + label 11px centré. Actif = `text-black`/`text-white`, inactif = `text-black/40`/`text-white/40`.
